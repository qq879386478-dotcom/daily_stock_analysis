...
> 其实就三步：先开启 Web 登录认证，再让服务监听外网，最后在浏览器里输入地址。
...
把 Web/API 暴露到公网前，必须先在 `.env` 中启用管理员认证：

```env
ADMIN_AUTH_ENABLED=true
```

重启服务后，第一次访问网页时会要求设置初始密码。未开启认证时，如果监听地址是 `0.0.0.0` / `::`，服务会拒绝启动；只有可信内网或临时调试场景才可以显式设置 `DSA_ALLOW_INSECURE_PUBLIC_API=true` 绕过。
...
> **注意**：未显式传入 `--host` 时，`python main.py` 会读取 `.env` 里的 `WEBUI_HOST`；如果你显式传入 `--host 0.0.0.0`，则以命令行参数为准。
...
Docker server 服务会在容器内监听 `0.0.0.0` 并映射到宿主机端口，因此 `.env` 必须先启用：

```env
ADMIN_AUTH_ENABLED=true
```

如果 `.env` 中仍是 `ADMIN_AUTH_ENABLED=false`，默认 Docker/API 启动会 fail closed，避免把系统配置、调度和分析接口无认证暴露到公网。只有可信内网或临时调试场景才建议显式设置 `DSA_ALLOW_INSECURE_PUBLIC_API=true`。
...
放行前请先确认 `.env` 已设置 `ADMIN_AUTH_ENABLED=true`，并完成首次密码设置；否则公网监听会被启动 guard 拒绝，避免无认证暴露管理 API。
...
重启服务后，第一次访问网页时会要求设置初始密码。设置完成后，每次打开设置页面都需要输入密码，可以防止 API Key 等敏感配置被他人看到。若确实需要在可信内网临时关闭认证并监听公网地址，必须显式设置 `DSA_ALLOW_INSECURE_PUBLIC_API=true`，并自行保证网络边界。
