...
- 旧配置迁移：如果现有 `.env` 明确保留旧的 `ALPHASIFT_INSTALL_SPEC=git+https://github.com/ZhuLinsen/alphasift.git@de54ea0da367be85770d9589a5bf7ded4f62d386`，该显式环境变量会覆盖代码默认值；升级热点详情缓存契约时应删除该行以使用内置默认值，或同步改为 `14e74fc0819267f7c04c3117a0dd0fe3f9b19404` 后重新安装依赖/重建后端。
