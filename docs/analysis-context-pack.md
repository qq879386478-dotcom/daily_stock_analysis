...
**Background（PR 描述对齐）：**`#1381` 聚焦单股分析的当日大盘上下文复用与回退控制，不改变现有日内阶段、日报或状态建模架构。该说明与当前 diff 与 `docs/CHANGELOG.md` [Unreleased] 中的 #1381 条目保持一致。
**Scope（本轮实现范围）：**`#1381` 仅覆盖后端大盘上下文注入、当日/目标交易日复用控制与保守护栏；本轮不包含独立 API、Web 阶段结果独立展示、四阶段日报结构化持久化或新增日报状态表；未落地项已移至后续计划/Issue。范围对应目录与入口为 `main.py`（调度与 `--no-market-review`）、`src/core/pipeline.py`、`src/services/daily_market_context.py`、`src/analyzer.py`、`src/agent/*.py`、`src/daily_market_context_guardrail.py`。本轮没有 `api/**`、`apps/dsa-web/**`、`apps/dsa-desktop/**` 或数据库迁移改动，故不应按 Web/API 阶段验收。  
**Acceptance Criteria（验收边界）：**当前 PR 的验收边界只包含 `tests/test_main_schedule_mode.py`、`tests/test_daily_market_context.py`、`tests/test_daily_market_context_guardrail.py`、`tests/test_agent_executor.py`。重点覆盖项为：`--no-market-review` 禁止触发大盘复盘生成、单次 schedule 复用同一 `target_date`、多市场上下文加载（`cn,us`）、`daily_market_context` 只在同一次分析主链路注入一次、普通分析与 Agent 路径应用护栏且不泄漏原始 `market_review_payload`。额外说明：当前无前端展示或独立 API 变更，CI 的 web-gate 应保持 skipped。  
**Compatibility/Risk（兼容与风险）：**`#1381` 不改变 `provider/model/base_url`、`默认模型`、配置清理/回填迁移语义；不新增数据库或运行时配置表变更；回滚方式为常规发布回滚（撤销本次相关代码），已落库历史 `analysis_context`（含历史 `MARKET` 记录）保持现网可读兼容；必要时配合重启并清理 runtime 覆盖项，恢复用户保存值。  
**兼容性证据：**本轮未新增 provider/model/base_url 迁移逻辑、默认模型回退/覆盖逻辑或配置清理逻辑；核验路径为：仅通过 `src/core/pipeline.py` / `src/services/daily_market_context.py` / `src/analyzer.py` / `main.py` 读取并复用既有 `LLM` 配置链路；未触达 `main` 配置持久化、系统设置写入、`src/config.py` 的保存/迁移分支；未新增/改写 `.env`、`系统设置` 或数据库迁移；回滚路径同样为常规发布回滚，必要时配合重启/清理 runtime 覆盖项恢复用户保存值（与现网既有说明一致）。  
